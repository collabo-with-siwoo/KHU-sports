...
export function LeaderboardFilters() {
  ...
  function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    if (formData.get("finalOnly") === "on") {
      nextParams.set("finalOnly", "true");
    } else {
      nextParams.delete("finalOnly");
    }
  ...
  return (
    <details className="score-filter-panel" open>
      ...
      <form className="leaderboard-filter-form" onSubmit={submitSearch}>
        <label>
          <span>선수명</span>
          <input defaultValue={searchParams.get("name") ?? ""} name="name" placeholder="이름 일부 검색" type="search" />
        </label>
