...
- Session persistence: middleware refreshes Supabase SSR cookies on auth-sensitive routes (`/admin`, `/mypage`, and auth/logout routes) so member and admin login persist until explicit logout without adding auth-server latency to public pages. The app also writes an HTTP-only `khu_app_session_started_at` marker cookie, but middleware must not use that marker to force Supabase sign-out. The marker defaults to Supabase's long-lived cookie window, 400 days, when `SESSION_MAX_AGE_HOURS` is unset.
...
- The admin notice form shows selected file usage immediately: promotion image size against the 10MB image limit, document attachment totals against the 8MB request limit, each document size against the 50MB document limit, and total selected upload size against the 8MB request limit.
- Public notice detail pages render promotion images at the article width with natural image height so tall poster assets are not cropped.
...
## M7 Beta Security Contracts

- Member login, admin login, signup, password reset, and player score submission are protected by app-level fixed-window rate limits.
...
- `POST /logout` uses the same same-origin guard before Supabase sign-out and app-session cookie clearing.
