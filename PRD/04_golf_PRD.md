...
>
> **v0.4 주요 변경**
> - 참가 신청 방식 변경: 이메일 접수 → **사이트 내 로그인 후 참가 신청** (온라인 접수)
> - 참가 신청 흐름: 로그인 → 마이페이지 "참가 신청" → 대회 선택 → 서류 업로드 → 어드민 승인
> - 경기 결과 공개 정책 변경: `/results/[tournamentId]`에서 **Full Leaderboard + 공개 Scorecard 탭** 제공. 공개 Scorecard는 경기 기록 필드만 노출하고 개인정보·관리자 메모·검수 로그는 비공개.
>
> **v0.3 주요 변경**
...
> - 회원 라이프사이클 정책 명시 (휴면은 선택 운영 정책으로 재정의, 탈퇴 30일 유예 후 삭제 + 익명화)
> - 약관 변경 처리 가이드 (약관/개인정보 처리방침 변경 고지 기준) 어드민 페이지에 명시
...
| 대회 결과 목록 | 진행 중/종료된 대회 리스트 (대회명·장소·기간·상태·결과 보기) | P0 |
| 대회 결과 상세 | **공개**: Full Leaderboard + Scorecard 탭. 순위·선수명·학교·참가구분·성별·라운드별 스코어·36홀 합계·최종순위·조·출발시간 공개. 개인정보/메모/검수 로그 비공개 | P0 |
| 마이페이지 | 본인 정보 수정, 비밀번호 변경, 회원 탈퇴 | P0 |
| 마이페이지 - 내 기록 | **선수(PLAYER)만**: 참가 대회 목록 + 스코어카드 상세 + 누적 승률 | P0 |
| 참가 신청 | 로그인 후 마이페이지에서 **대회 선택 → 참가 신청서 작성 → 서류 업로드 → 어드민 승인** | P0 |

> **중요 정책**:
> - 가입 직후 모든 회원은 **일반 가입자(GENERAL)**. 로그인 후 사이트 내 참가 신청 → 어드민 승인 후 PLAYER로 전환.
> - 경기 결과 페이지의 공개 Scorecard는 경기 기록 전용 화면이다. 전화번호·이메일·생년월일·주소·보호자 연락처·선수 메모·관리자 메모·관리자 검수 로그는 절대 노출하지 않는다.
...
**(a) 회원 탈퇴·휴면 처리 가이드**
- 휴면: 개인정보 유효기간제 폐지 이후 1년 미접속 자동 휴면은 법정 의무로 단정하지 않는다. 서비스 보호 목적의 선택 운영 정책으로만 둘 수 있으며, 적용 시 개인정보 처리방침과 운영 가이드에 명시한다.
...
가입 직후 모든 회원은 **일반 가입자(GENERAL)**입니다. 선수로 활동하려면 사이트 내 참가 신청 절차가 필요합니다.

**유저 측 흐름:**
1. 로그인 후 마이페이지에서 **"참가 신청"** 버튼 클릭
2. 참가 신청 페이지:
   - 참가할 대회 선택
   - 참가 신청서 작성 (선수 정보, 소속 등)
   - 필요 서류 업로드 (서약서, 동의서 등 — R2 Presigned URL)
3. 신청 완료 후 **대기(PENDING)** 상태로 전환

**어드민 측 흐름:**
1. 어드민 > 참가 신청 관리에서 접수된 신청 목록 확인
2. 업로드된 서류 검토
3. 승인 시 회원 유형을 **일반(GENERAL) → 선수(PLAYER)**로 변경
...
|---|---|---|
| **ACTIVE** | 가입 직후 | 정상 이용 |
| **DORMANT** (휴면) | 운영자가 선택 정책으로 적용할 때의 장기 미접속/보호 필요 상태 | 로그인 시도 시 휴면 해제 안내, 본인 인증 후 재활성 |
| **WITHDRAWN_PENDING** | 탈퇴 신청 | **30일 유예** — 복구 요청 시 ACTIVE 복구 |
| **WITHDRAWN_DELETED** | 탈퇴 후 30일 경과 | 개인정보 완전 삭제 (이름·이메일·전화·생년) |

### 4.5.1 휴면 정책 상세
- 개인정보 유효기간제 폐지 이후, **335일 안내/365일 자동 휴면 전환은 법정 의무가 아닌 선택 운영 정책**으로 취급한다.
- MVP에서는 자동 휴면 전환을 기본 구현 범위에서 제외하고, `DORMANT` 상태는 향후 운영 정책 확정 시 사용할 예약 상태로 둔다.
- 장기 미접속자 안내 메일을 운영 정책으로 도입할 경우, 목적·기준·복구 절차를 개인정보 처리방침과 어드민 운영 가이드에 명시한다.
- 휴면 계정을 운영할 경우 별도 보관 또는 목록 제외 기준을 스펙에 명확히 기록한다.
- 복구 절차: 본인 이메일 인증
...
| 장기 미접속 안내 | 휴면 또는 계정 보호 정책 안내 | 선택 운영 정책 |
| 탈퇴 완료 | 탈퇴 확인 메일 | 권장 |
...
| 역할 | 범위 |
|---|---|
| **공개 (비로그인)** | 메인홈, 공지 목록·상세, **대회 결과 목록·상세 (Full Leaderboard + 공개 Scorecard 경기 기록)** |
| **로그인 사용자** | + 마이페이지, **본인의 스코어카드·승률·참가 대회 상세** |
| **어드민 멤버** | + 부여받은 메뉴, **모든 선수의 스코어카드·기록 조회** |
| **슈퍼어드민** | + 전체 메뉴, 관리자 초대·권한 부여 |

> **핵심 제약**: 공개 결과 화면은 경기 기록 필드만 노출한다. 공개 가능: 순위, 선수명, 학교, 참가구분, 성별, 라운드별 스코어, 36홀 합계, 최종순위, 조, 출발시간. 비공개: 전화번호, 이메일, 생년월일, 주소, 보호자 연락처, 선수 메모, 관리자 메모, 관리자 검수 로그.
...
| 본인 외 스코어 조회 | **공개 경기 기록은 가능**, 개인정보/메모/검수 로그는 불가 |
| 대회 결과 공개 범위 | `/results/[tournamentId]`에서 Full Leaderboard와 공개 Scorecard 제공. 순위·선수명·학교·참가구분·성별·라운드별 스코어·36홀 합계·최종순위·조·출발시간 공개 |
| 휴면 | 개인정보 유효기간제 폐지 반영: **법정 의무가 아닌 선택 운영 정책**, MVP 자동 전환 제외 |
| 탈퇴 | **30일 유예 후 개인정보 삭제**, 스코어는 익명화 보존 |
| 약관 변경 처리 | 일반 7일 / 불리한 변경 30일 + 재동의 기준을 운영 가이드에 명시 |
...
| 장기 미접속 안내 메일 자동 발송 | 휴면 정책 확정 후 도입 검토 |
